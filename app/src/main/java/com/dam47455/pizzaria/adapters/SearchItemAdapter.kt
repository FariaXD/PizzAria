...
import com.dam47455.pizzaria.fragments.HomeFragment
import com.dam47455.pizzaria.fragments.SearchFragment
import com.squareup.picasso.Picasso

class SearchItemAdapter(context: Context, var data: ArrayList<Food>, var search: SearchFragment ?= null, var home: HomeFragment ?= null) : ArrayAdapter<Food>(context, 0, data) {
    @SuppressLint("InflateParams")
    override fun getView(position: Int, itemView: View?, parent: ViewGroup): View {
        val view = LayoutInflater.from(context).inflate(R.layout.food_item, null, false)
        ...
        view.findViewById<Button>(R.id.addButton).setOnClickListener{
            if(search == null)
                home!!.addItemToCart(data[position])
            else
                search!!.addItemToCart(data[position])
        }
